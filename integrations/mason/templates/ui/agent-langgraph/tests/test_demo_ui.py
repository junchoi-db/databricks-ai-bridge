...
async def _session_history(routing_session, state_session_id, request_auth):
    del state_session_id, request_auth
    return {
        "session_id": routing_session,
        "session_items": [
            {"item_id": "1", "data": {"type": "human", "content": routing_session}},
...
@pytest.mark.asyncio
async def test_user_auth_checkpoint_history_partitions_same_routing_cookie(
    ...
    assert configs == [
        {"configurable": {"thread_id": state_key, "actor_id": state_key}}
        for state_key in state_keys
    ]
    assert [context.state_key("shared-route") for context in contexts] == state_keys
...
async def test_user_auth_managed_session_access_uses_principal_state_key(
    ...
    def state_result(session_id, *, include_items=False):
        result = {
            ...
            "metadata": {
                ...
                f"thread:{session_id}": f"session {session_id} ready",
                "nested": [
                    {
                        "actor_id": session_id,
    ...
    for index, response in enumerate(responses):
        ...
        assert body["metadata"] == {
            "langgraph_thread_id": "shared-route",
            "thread:shared-route": "session shared-route ready",
            "nested": [
                {
                    "actor_id": "shared-route",
...
async def test_deployed_user_auth_missing_token_rejected_before_history_access(
    ...
    if managed:
        ...
        async def checkpoint_history(routing_session, state_session_id, request_auth):
            accesses.append(("local", routing_session, state_session_id, request_auth))
            return {"session_id": routing_session, "session_items": [], "interrupts": []}

        monkeypatch.setattr(ui, "_checkpoint_history", checkpoint_history)
...
async def test_checkpoint_history_reads_messages_and_interrupts(monkeypatch):
    ...
    async def fake_create_agent_graph(actual_request_auth):
        assert actual_request_auth is request_auth
        return FakeAgent()

    monkeypatch.setattr(agent_module, "create_agent_graph", fake_create_agent_graph)
    result = await ui._checkpoint_history("saved-session", "saved-session", request_auth)
